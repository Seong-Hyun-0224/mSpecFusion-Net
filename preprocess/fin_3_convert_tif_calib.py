...
def converttif(path_dir):
    ...
    path_out_dir = 'data/{}_calib'.format(path_dir)
    fac_chs = [1, 1, 1, 1.875001172,1.507537688, 3.618158686, 4.048927006, 2.419354839, 9.847245868, 2.112676056, 2.052831156, 1, 1, 1, 1, 2.04620371, 1.614978787, 5.088322542, 4.805588286, 2.639070553, 11.53846154, 2.112676056, 3.142412755]
            ...
            for id in ids:
                ...
                for idx, ch in enumerate(chs):
                    ...
                    if idx == 0:
                        ...
                        set_temp[:,:,idx + 2] = np.mean(img_, axis=2) * calib * fac_chs[idx]
                        # set_temp[:,:,idx + 2] = np.mean(img_, axis=2)
                    elif idx == 9:
                        set_temp[:,:,idx + 2] = np.mean(img_, axis=2)
                    elif idx == 10:
                        set_temp[:,:,12] = img_[:,:,0]
                        ...
                        # set_temp[:,:,idx + 4] = np.mean(img_, axis=2)
                        set_temp[:,:,idx + 4] = np.mean(img_, axis=2) * pcalib * fac_chs[idx]
...
for cor in corrections:
    converttif('origin/20230713_all_size128_album_{}'.format(cor))